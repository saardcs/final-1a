...
def render_custom_component(item, components):
    ...
    if component_name in components:
        ...
        component_results[component_name] = component(key=item["id"])
    else:
...
def render_lcm(item):
    ...
    st.text_area(
        f"Multiples of {num1}:",
        placeholder=f"e.g., {num1}, {num1 * 2}...",
        key=f"{item['id']}_multiples_num1"
    )

    st.text_area(
        f"Multiples of {num2}:",
        placeholder=f"e.g., {num2}, {num2 * 2}...",
        key=f"{item['id']}_multiples_num2"
    )

    st.text_input(
        "LCM:",
        placeholder="Enter a whole number (e.g., 24)",
        key=f"{item['id']}_lcm"
    )

def render_binary_image(item):
    st.write(item["text"])

    global component_results

    component_results[item["component"]] = binary_grid(
        key=item["id"]
    )
...
sudoku2 = components.declare_component("sudoku2", path="components/sudoku2")
tree = components.declare_component("tree", path="components/tree")
binary_grid = components.declare_component(
    "binary_grid",
    path="components/binary_grid"
)
ascii_encode = components.declare_component(
    "ascii_encode",
    path="components/ascii_encode"
)
ascii_decode = components.declare_component(
    "ascii_decode",
    path="components/ascii_decode"
)
custom_components = {
    "sudoku": sudoku,
    "sudoku2": sudoku2,
...
def grade_lcm(raw_lcm, user_raw_n1, user_raw_n2, num1, num2, max_points):
    ...
    user_multiples_n1 = extract_numbers(user_raw_n1)
    user_multiples_n2 = extract_numbers(user_raw_n2)

    correct_lcm = math.lcm(num1, num2)

    # All correct multiples up to and including the LCM
    correct_n1 = {
        num1 * i
        for i in range(1, correct_lcm // num1 + 1)
    }

    correct_n2 = {
        num2 * i
        for i in range(1, correct_lcm // num2 + 1)
    }

    # Give credit for each correct multiple.
    # Wrong numbers receive no credit, but are NOT negatively marked.
    def score_for_side(user_multiples, correct_multiples):

        if not user_multiples:
            return 0

        correct = len(user_multiples & correct_multiples)
        total = len(correct_multiples)

        return correct / total

    score_n1 = score_for_side(user_multiples_n1, correct_n1)
    score_n2 = score_for_side(user_multiples_n2, correct_n2)

    # LCM answer = half the marks
    lcm_score = 0
    lcm_values = extract_numbers(raw_lcm)

    if correct_lcm in lcm_values:
        lcm_score = max_points / 2

    # Multiples = other half
    multiples_score = (
        (score_n1 + score_n2) / 2
    ) * (max_points / 2)

    return round(multiples_score + lcm_score, 2)


def grade_binary_image(result, values, max_points):
    if not result:
        return 0
    ...
    for i in range(len(values)):
        ...
        if i < len(pixels):
            pixel_binary = "".join(
                "1" if cell else "0"
                for cell in pixels[i]
            )

            if pixel_binary == correct[i]:
                correct_count += 1

        if i < len(binaries):
            if str(binaries[i]).strip() == correct[i]:
                correct_count += 1

    return round((correct_count / total) * max_points, 2)


def grade_ascii_encode(result, item, max_points):
    if not result:
        return 0
    ...
    correct_binary = [
        format(decimal, "08b")
        for decimal in correct_decimal
    ]
    ...
    for i in range(len(letters)):
        ...
        if i < len(decimals):
            try:
                if int(str(decimals[i]).strip()) == correct_decimal[i]:
                    correct_count += 1
            except (ValueError, TypeError):
                pass

        # 2. Clickable binary grid
        if i < len(pixels):
            pixel_binary = "".join(
                "1" if cell else "0"
                for cell in pixels[i]
            ...
            if pixel_binary == correct_binary[i]:
                correct_count += 1

        # 3. Typed binary
        if i < len(binaries):
            if str(binaries[i]).strip() == correct_binary[i]:
                correct_count += 1

    return round(
        (correct_count / total) * max_points,
        2
    )


def grade_ascii_decode(result, item, max_points):
    if not result:
        return 0

    correct_binary = item["binary"]

    correct_decimal = [
        int(binary, 2)
        for binary in correct_binary
    ]
    ...
    correct_count = 0
    total = len(correct_binary) * 3

    for i in range(len(correct_binary)):

        # Check clickable grid
        if i < len(pixels):
            pixel_binary = "".join(
                "1" if cell else "0"
                for cell in pixels[i]
        ...
        if i < len(decimals):
            try:
                if int(decimals[i]) == correct_decimal[i]:
                    correct_count += 1
            except:
                pass
        ...
        # Check letter
        if i < len(letters):
            if str(letters[i]).strip().upper() == correct_letters[i]:
                correct_count += 1
...
def grade_binary_conversion(answer, item, max_points):
    if not answer:
        return 0

    answer = str(answer).strip().replace(" ", "")

    if item["direction"] == "decimal_to_binary":
        try:
            user_value = int(answer, 2)
        except ValueError:
            return 0

        correct_value = item["decimal"]

    elif item["direction"] == "binary_to_decimal":
        try:
            user_value = int(answer)
        except ValueError:
            return 0

        correct_value = int(item["binary"], 2)

    else:
        return 0

    return max_points if user_value == correct_value else 0

def grade_tree(result, max_points):
    if not result:
        return 0

    answers = st.secrets["tree"]

    correct_count = 0
    total = 10

    # Tree 1 — missing middle node
    tree1 = result.get("tree1", {})

    if str(tree1.get("node2", "")).strip() == answers["tree1_node2"]:
        correct_count += 1

    # Tree 1 — leaves, order does NOT matter
    user_leaves = [
        str(tree1.get(f"leaf{i}", "")).strip()
        for i in range(1, 5)
    ]

    remaining = user_leaves.copy()

    for answer in answers["tree1_leaves"]:
        if answer in remaining:
            correct_count += 1
            remaining.remove(answer)


    # Tree 2 — missing middle node
    tree2 = result.get("tree2", {})

    if str(tree2.get("node2", "")).strip() == answers["tree2_node2"]:
        correct_count += 1

    # Tree 2 — leaves, order does NOT matter
    user_leaves = [
        str(tree2.get(f"leaf{i}", "")).strip()
        for i in range(1, 5)
    ]

    remaining = user_leaves.copy()

    for answer in answers["tree2_leaves"]:
        if answer in remaining:
            correct_count += 1
            remaining.remove(answer)
    ...
    for section in exam_data.get("sections", []):
        ...
        for item in section.get("items", []):
            ...
            if item_type == "mcq":
                ...
                puzzle = st.secrets[component_name]["puzzle"]
                solution = st.secrets[component_name]["solution"]
                board_size = len(puzzle)

                user_board = component_results.get(component_name, [])
                ...
                raw_multiples1 = st.session_state.get(
                    f"{item_id}_multiples_num1", ""
                )
                raw_multiples2 = st.session_state.get(
                    f"{item_id}_multiples_num2", ""
                )
                raw_lcm = st.session_state.get(
                    f"{item_id}_lcm", ""
                )
                ...
                submission["answers"][section_name][item_id] = {
                    "multiples_num1": raw_multiples1,
                    "multiples_num2": raw_multiples2,
                    "lcm": raw_lcm,
                    "type": item_type,
                    "score": score
                }
            ...
            elif item_type == "binary_image":

                component_name = item["component"]

                result = component_results.get(component_name, {})

                score = grade_binary_image(
                    result,
                    item["values"],
                    max_points
                )
            ...
            elif item_type == "ascii_encode":
                component_name = item["component"]
                result = component_results.get(component_name, {})

                score = grade_ascii_encode(
                    result,
                    item,
                    max_points
                )
            ...
            elif item_type == "ascii_decode":
                component_name = item["component"]
                result = component_results.get(component_name, {})

                score = grade_ascii_decode(
                    result,
                    item,
                    max_points
                )
            ...
            elif item_type == "binary_conversion":
                user_answer = st.session_state.get(item_id, "")

                score = grade_binary_conversion(
                    user_answer,
                    item,
                    max_points
                )
                ...
                result = component_results.get(item["component"])
